  ...
  Task.find()
    ...
    .then((returnedTasks) => {
      ...
      for(var i = 0; i < returnedTasks.length; i++){
        response += `{"status": "${returnedTasks[i].status}", "description": "${returnedTasks[i].description}", "user": "${returnedTasks[i].user.name}"}`;
        if(i < returnedTasks.length - 1) response += ',';
      }
      response += ']';
      res.end(response);
    })
    ...
    model.find()
      .then((returned) => {
        ...
        for(var i = 0; i < returned.length; i++){
          response += `{"_id": "${returned[i]._id}", "name": "${returned[i].name}"}`;
          if(i < returned.length - 1) response += ',';
        }
        response += ']';
        res.end(response);
      })
